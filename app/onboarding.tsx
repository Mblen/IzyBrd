...
export default function OnboardingScreen() {
  ...
  // Handle availability, checked while they type rather than on submit.
  const [handleState, setHandleState] = useState<'idle' | 'checking' | 'free' | 'taken'>('idle');
  const [suggestions, setSuggestions] = useState<string[]>([]);

  useEffect(() => {
    if (!isSupabaseConfigured || username.length < 3) {
      setHandleState('idle');
      setSuggestions([]);
      return;
    }
  ...
  // The school is optional; only the handle is required - and it has to be free.
  const canContinue =
    step !== 2 ||
    (username.length >= 3 && handleState !== 'taken' && handleState !== 'checking');
